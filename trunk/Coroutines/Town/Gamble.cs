...
namespace Trinity.Coroutines.Town
{
    public class Gamble
    {
        ...
        public static void CheckShouldTownRunForGambling()
        {
            ...
            if (Trinity.Settings.Gambling.ShouldTownRun && ZetaDia.PlayerData.BloodshardCount >= Math.Min(Trinity.Settings.Gambling.SaveShardsThreshold, Trinity.Player.MaxBloodShards))
            {
        ...
        private static bool CanBuyItems
        {
            get
            {
                try
                {                                  
                    if (!ZetaDia.IsInTown || ZetaDia.WorldType != Act.OpenWorld || Trinity.Player.IsCastingOrLoading)
                        return false;
                    ...
                    if (ZetaDia.PlayerData.BloodshardCount < Trinity.Settings.Gambling.MinimumBloodShards || !CanAffordMostExpensiveItem)
                    {
        ...
        private static async Task<bool> BuyItem()
        {
            try
            {
                ...
                if (!_gambleRotation.Any())
                    _gambleRotation = Trinity.Settings.Gambling.SelectedGambleSlots;
        ...
        public static bool CanRun(bool ignoreSaveThreshold = false)
        {
            ...
            try
            {
                if (ZetaDia.WorldType != Act.OpenWorld || Trinity.Player.IsCastingOrLoading)
                {
                    return false;
                }
                ...
                if (Trinity.Settings.Gambling.SelectedGambleSlots.Count <= 0)
                {
        ...
        private static void LogVerbose(string msg, params object[] args)
        {
            var debugInfo = string.Format(" Shards={0} SaveShards={1} SaveThreshold={2} CanAffordItem={3} SelectedSlots={4}",
                ...
                Trinity.Settings.Gambling.ShouldSaveShards,
                Math.Min(Trinity.Settings.Gambling.SaveShardsThreshold, Trinity.Player.MaxBloodShards),
                CanAffordMostExpensiveItem,
                Trinity.Settings.Gambling.SelectedGambleSlots.Count);
        ...
        public static bool CanAffordMostExpensiveItem
        {
            get
            {
                var slotAndPrice = TownInfo.MysterySlotTypeAndPrice.Where(pair => Trinity.Settings.Gambling.SelectedGambleSlots.Contains(pair.Key)).ToList();
        ...
        {
            get { return ZetaDia.PlayerData.BloodshardCount < Trinity.Settings.Gambling.MinimumBloodShards; }
        }
        ...
        private static bool ShouldSaveShards
        {
            get
            {
                if (Trinity.Settings.Gambling.ShouldSaveShards && ZetaDia.PlayerData.BloodshardCount < Math.Min(Trinity.Settings.Gambling.SaveShardsThreshold, Trinity.Player.MaxBloodShards))
                {
