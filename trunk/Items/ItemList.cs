...
namespace Trinity.Items
{
    public class ItemList
    {
        ...
        {
            var typeEntry = Trinity.Settings.Loot.ItemList.GetitemTypeRule(cItem.TrinityItemType);
        ...
        internal static bool ShouldStashItem(Item referenceItem, CachedItem cItem, bool test = false)
        {
            ...
            if (test)
            {
                ...
                foreach (var prop in props)
                {
                    ...
                    EvaluateProperty(cItem, prop, (float)range.AncientMax, 0, RuleType.Test);
                }
        ...
        private static bool EvaluateRules(CachedItem cItem, LItem itemSetting, bool isTest)
        {
            ...
            {
                if (!EvaluateProperty(itemRule, cItem))
                {
            ...
            foreach (var itemRule in itemSetting.OptionalRules)
            {
                ...
                if (trueOptionals >= itemSetting.Ops && !isTest)
                    return true;
            }
        ...
        internal static bool EvaluateProperty(LRule itemRule, CachedItem cItem)
        {
            return EvaluateProperty(cItem, itemRule.ItemProperty, (float)itemRule.Value, itemRule.Variant, itemRule.RuleType);
        }

        private static bool EvaluateProperty(CachedItem item, ItemProperty prop, float value, int variant, RuleType ruleType = RuleType.Test)
        {
            var result = false;
            ...
            switch (prop)
            {
                ...
                case ItemProperty.SkillDamage:
                    ...
                    var skillId = variant;
                    var skill = ItemDataUtils.GetSkillsForItemType(item.TrinityItemType, Trinity.Player.ActorClass).FirstOrDefault(s => s.Id == skillId);
                    if (skill != null)
                    {
