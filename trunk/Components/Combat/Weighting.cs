...
namespace Trinity.Components.Combat
{
    public class Weighting
        {
            public void RefreshDiaGetWeights(List<TrinityActor> objects)
            {
                using (new PerformanceLogger("RefreshDiaObjectCache.Weighting"))
                {
                    ...
                    var healthGlobeEmergency = (Core.Player.CurrentHealthPct <= CombatBase.EmergencyHealthGlobeLimit ||
                                                Core.Player.PrimaryResourcePct <= CombatBase.HealthGlobeResource &&
                    ...
                    var hiPriorityShrine = Core.Settings.WorldObject.HiPriorityShrines;

                    var getHiPriorityShrine = objects.Any(s => s.Type == TrinityObjectType.Shrine) &&
                                              hiPriorityShrine;

                    var getHiPriorityContainer = Core.Settings.WorldObject.HiPriorityContainers &&
                                                 objects.Any(c => c.Type == TrinityObjectType.Container);

                    var isGateNearby = Core.Actors.AllRActors.Any(r => r.Distance <= 40f && r.ActorSnoId == (int) SNOActor.x1_Fortress_Portal_Switch);
                ...
                var profileTagCheck = false;
                    ...
                    Trinity.TrinityPlugin.HighestWeightFound = 0;

                    var isStuck = Navigator.StuckHandler.IsStuck;

                    var elites = new List<TrinityActor>();
                    var eliteswithAffixes = new List<TrinityActor>();

                    var ignoredAffixes = Core.Settings.Combat.Misc.IgnoreAffixes.GetFlags<MonsterAffixes>().ToList();
                    ...
                    if (Core.Settings.Combat.Misc.GoblinPriority == GoblinPriority.Kamikaze)
                    {
                        ...
                        if (goblin != null && !isStuck && !PlayerMover.IsCompletelyBlocked && !Core.Grids.Avoidance.IsIntersectedByFlags(Core.Player.Position, goblin.Position, AvoidanceFlags.ClosedDoor))
                        {
                            ...
                            Trinity.TrinityPlugin.CurrentTarget = goblin;
                        }
                        else
                        {
                    ...
                    foreach (var cacheObject in objects.Where(x => !x.IsPlayer))
                    {
                        ...
                        {
                            if (cacheObject.RActorId == CombatBase.KamakaziGoblin.RActorId && !isStuck)
                            {
                            ...
                            case TrinityObjectType.Unit:
                                {
                                        ...
                                        if (ignoreTrashTooFarAway || ignoreElitesTooFarAway)
                                        {
                                            cacheObject.WeightInfo +=
                                                string.Format("Ignore Far Away Stuff TrashRange={0} EliteRange={1}",
                                                    Core.Settings.Combat.Misc.NonEliteRange, Core.Settings.Combat.Misc.EliteRange);
                                            cacheObject.Weight = 0;
                                            break;
                                        }
                                    ...
                                    if (!cacheObject.IsBoss && !cacheObject.IsTreasureGoblin &&
                                        Trinity.TrinityPlugin.LastTargetRactorGUID != cacheObject.RActorId &&
                                    ...
                                    else if (cacheObject.Distance < 25 && Core.Player.IsCastingTownPortalOrTeleport() &&
                                    ...
                                    else if (cacheObject.IsTrashMob)
                                    {
                                        ...
                                        else if (cacheObject.IsSummoner && Core.Settings.Combat.Misc.ForceKillSummoners)
                                        {
                                            cacheObject.WeightInfo +=
                                    ...
                                    else if (isUnique || isBoss || isRare || isMinion || isChampion)
                                    {
                                        ...
                                        else if ((Core.Settings.Combat.Misc.IgnoreElites ||
                                                  Core.Settings.Combat.Misc.IgnoreRares && isRare ||
                                                  Core.Settings.Combat.Misc.IgnoreMinions && isMinion ||
                                                  Core.Settings.Combat.Misc.IgnoreChampions && isChampion) &&
                                                 !cacheObject.IsBoss)
                                        {
                                            cacheObject.WeightInfo +=
                                                string.Format(
                                        ...
                                        else if (Core.Settings.Combat.Misc.IgnoreHighHitPointElites)
                                        {
                                            HashSet<string> highHitPointTrashMobNames = new HashSet<string>
                                            {
                                                "mallet", //
                            ...
                            case TrinityObjectType.Item:
                                {
                                    ...
                                    if (cacheObject.ActorSnoId == 361989 || cacheObject.ActorSnoId == 449044)
                                    {
                                        ...
                                        if (Core.Settings.Loot.Pickup.IgnoreNonLegendaryInAoE && Core.Avoidance.InAvoidance(cacheObject.Position) ||
                                    ...
                                    if (cacheObject.ItemQualityLevel < ItemQuality.Legendary)
                                    {
                                        ...
                                        if (Core.Settings.Loot.Pickup.IgnoreNonLegendaryNearElites &&
                                            objects.Any(
                                                u =>
                                                    u.IsElite &&
                                                    u.Position.Distance(cacheObject.Position) <= 15f))
                            ...
                            case TrinityObjectType.Gold:
                                {
                                    ...
                                    if (Core.Settings.Loot.Pickup.IgnoreGoldNearElites &&
                                        objects.Any(
                                            u =>
                                                u.IsElite &&
                                                u.Position.Distance(cacheObject.Position) <= 15f))
                                    ...
                                    // Ignore gold in AoE
                                    if (Core.Settings.Loot.Pickup.IgnoreGoldInAoE && Core.Avoidance.Grid.IsLocationInFlags(cacheObject.Position, AvoidanceFlags.Avoidance))
                                    {
                                        break;
                                    }
                            ...
                            case TrinityObjectType.PowerGlobe:
                                {
                                    ...
                                    if (Core.Settings.Loot.Pickup.IgnoreNonLegendaryInAoE && Core.Avoidance.Grid.IsLocationInFlags(cacheObject.Position, AvoidanceFlags.Avoidance))
                                    {
                            ...
                            case TrinityObjectType.ProgressionGlobe:
                                {
                                    ...
                                    if (Core.Settings.Loot.Pickup.IgnoreProgressionGlobesInAoE && Core.Avoidance.Grid.IsLocationInFlags(cacheObject.Position, AvoidanceFlags.CriticalAvoidance))
                                    {
                                        cacheObject.WeightInfo += $"Ignoring {cacheObject.InternalName} - In Avoidance.";
                                        break;
                                    }

                                    if (Core.Settings.Combat.Misc.IgnoreNormalProgressionGlobes && DataDictionary.NormalProgressionGlobeSNO.Contains(cacheObject.ActorSnoId))
                                    {
                                        cacheObject.WeightInfo += $"Ignoring {cacheObject.InternalName} because of settings.";
                                        break;
                                    }

                                    if (Core.Settings.Combat.Misc.IgnoreGreaterProgressionGlobes && DataDictionary.GreaterProgressionGlobeSNO.Contains(cacheObject.ActorSnoId))
                                    {
                                        cacheObject.WeightInfo += $"Ignoring {cacheObject.InternalName} because of settings.";
                                        break;
                                    }
                            ...
                            case TrinityObjectType.Interactable:
                                {
                                    ...
                                    if (DataDictionary.HighPriorityInteractables.Contains(cacheObject.ActorSnoId) &&
                            ...
                            case TrinityObjectType.Container:
                                {
                                    ...
                                    float maxRange = Core.Settings.WorldObject.ContainerOpenRange;
                                    var isRare = cacheObject.InternalName.ToLower().Contains("chest_rare") ||
                                                 DataDictionary.ResplendentChestIds.Contains(cacheObject.ActorSnoId);
                                    if (isRare)
                                        maxRange = 250f;
            ...
            private void SetTarget(TrinityActor bestTarget)
            {
                // Set Record History
                if (bestTarget?.InternalName != null && bestTarget.ActorSnoId > 0 && bestTarget.Weight > 0)
                {
                    ...
                    Trinity.TrinityPlugin.LastTargetIsSafeSpot = bestTarget != null && Trinity.TrinityPlugin.CurrentTarget != null && Trinity.TrinityPlugin.CurrentTarget.IsSafeSpot;
                    Trinity.TrinityPlugin.CurrentTarget = bestTarget;

                    //Logger.Log($"Last Guid = {LastTargetRactorGUID}");

                    var timesTargetted = RecordTargetHistory(bestTarget);

                    if (bestTarget.RActorId != Trinity.TrinityPlugin.LastTargetRactorGUID || bestTarget != null && bestTarget.IsMarker)
                    {
                        Logger.Log(TrinityLogLevel.Debug, LogCategory.UserInformation,
                            $"Target changed to {Trinity.TrinityPlugin.CurrentTarget.ActorSnoId} // {Trinity.TrinityPlugin.CurrentTarget.InternalName} RActorGuid={Trinity.TrinityPlugin.CurrentTarget.RActorId} " +
                            $"({Trinity.TrinityPlugin.CurrentTarget.Type}) {Trinity.TrinityPlugin.CurrentTarget.WeightInfo} TargetTimes={timesTargetted}");
                    }
                    return;
                }
            ...
            private TrinityActor GetNewBestTarget(TrinityActor cacheObject, TrinityActor bestTarget)
            {
                ...
                bestTarget = cacheObject;
                Trinity.TrinityPlugin.HighestWeightFound = cacheObject.Weight;
                return bestTarget;
            }
            ...
            public int RecordTargetHistory(TrinityActor bestTarget)
            {
                ...
                if (Core.Targets.TargetHistory.Count > 200)
                    Core.Targets.TargetHistory.RemoveAll(o => DateTime.UtcNow.Subtract(o.CreatedTime).TotalSeconds > 30);

                if (Core.Targets.TargetHistory.TryGetValue(bestTarget.ObjectHash, out info))
                {
                    var isNewTarget = bestTarget.ObjectHash != Trinity.TrinityPlugin.CurrentTarget.ObjectHash && !Trinity.TrinityPlugin.LastTargetIsSafeSpot && Trinity.TrinityPlugin.LastTargetRactorGUID > 0;
                    if (isNewTarget)
                    {
                ...
                Core.Targets.TargetHistory.Add(bestTarget.ObjectHash, new TargettingInfo
                {
            ...
            private double GetLastHadUnitsInSights()
            {
                return Math.Max(DateTime.UtcNow.Subtract(Trinity.TrinityPlugin.lastHadUnitInSights).TotalMilliseconds,
                    DateTime.UtcNow.Subtract(Trinity.TrinityPlugin.lastHadEliteUnitInSights).TotalMilliseconds);
            }
            ...
            /// <returns></returns>
            public double EliteMonsterNearFormula(TrinityActor cacheObject,
                List<TrinityActor> eliteMonsters)
            {
                double weight = 0;
                if (!Core.Settings.Combat.Misc.IgnoreElites && !Core.Settings.Combat.Misc.IgnoreChampions &&
                    !Core.Settings.Combat.Misc.IgnoreRares)
                    return 0;
            ...
            public double PackDensityFormula(TrinityActor cacheObject, List<TrinityActor> objects)
            {
                ...
                var pack = objects.Where(
                    x => x.IsUnit && x.IsHostile && x.Position.Distance(cacheObject.Position) < CombatBase.CombatOverrides.EffectiveTrashRadius && (!Core.Settings.Combat.Misc.IgnoreElites || !x.IsElite))
                    .ToList();
            ...
            public double PathBlockedFormula(TrinityActor cacheObject)
            {
                ...
                if (!cacheObject.IsUnit || PlayerMover.IsCompletelyBlocked && cacheObject.Distance > 15f && !cacheObject.IsElite && Core.Settings.Combat.Misc.AttackWhenBlocked)
                    return -MaxWeight;
