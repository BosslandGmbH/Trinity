...
{
    public class ActorAttributes2
    {
        private ACD _commonData;
        private FastAttribGroupsEntry _group;
        private IntPtr _mapPtr;
        private AttributeTable _table;

        public ActorAttributes2(ACD commonData)
        {
            Update(commonData);
        }
        ...
        public void Update(ACD commonData)
        {
            ...
            _group = ZetaDia.Storage.FastAttribGroups[commonData.FastAttribGroupId];
            _mapPtr = GetMapPtr(_group);
            _table = new AttributeTable(_mapPtr);
        }
        ...
        {
            if (!_commonData.IsValid) return default(T);
        ...
        {
            if (!_commonData.IsValid) return defaultProducer();
        ...
        {
            if (!_commonData.IsValid) return null;
        ...
        {
            if (!_commonData.IsValid) return Enumerable.Empty<T>();
        ...
        {
            if (!_commonData.IsValid) return new Dictionary<ActorAttributeType, float>();
        ...
        {
            return (group.Flags & 4) != 0 ? group.BaseAddress + 0x00C + 0x10 : group.BaseAddress + 0x010 + 0x10;
        }
        ...
        public class AttributeEntry : NativeObject
        {
            ...
            public override string ToString() => $"{Key.Descriptor}: Value={Value}";
        ...
        public class AttributeTable : NativeObject, IEnumerable<AttributeEntry>
        {
            ...
            public IntPtr DataPtr => ZetaDia.Memory.Read<IntPtr>(BaseAddress + 0x00);
            public int Size => ZetaDia.Memory.Read<int>(BaseAddress + 0x08);
        ...
        => Items.Aggregate($"Attributes {Environment.NewLine}", (c, a) => c + $" {a.Value} {Environment.NewLine}");
    }
