    ...
    public class WorldScene
    {
        ...
        public WorldScene(Scene scene, float boxSize, float boxTolerance)
        {
            using (new PerformanceLogger("[WorldScene] ctor", false))
            {
                ...
                Core.Logger.Verbose("[世界场景] 建立新的世界场景. 名称: {0} LevelArea: {1} ({2})", Name, (SNOLevelArea)LevelAreaId, LevelAreaId);
                if (LevelAreaId != AdvDia.CurrentLevelAreaId && !ExplorationData.OpenWorldIds.Contains(AdvDia.CurrentWorldId))
                {
                    Core.Logger.Verbose("[世界场景] 现在的LevelAreaID不同于CurrentLevelAreaID.");
                    Core.Logger.Verbose("[世界场景] 场景名称: {0}", Name);
                    Core.Logger.Verbose("[世界场景] 场景: {0} ({1})", (SNOLevelArea)LevelAreaId, LevelAreaId);
                    Core.Logger.Verbose("[世界场景] 当前: {0} ({1})", (SNOLevelArea)AdvDia.CurrentLevelAreaId, AdvDia.CurrentLevelAreaId);
                }
