...
namespace Trinity
{
    public class TrinityCacheObject
    {
        ...
        /// <summary>
        /// Retrieve a combination of cached and updated property values; 
        /// Performance is improved by only updating properties that change.
        /// </summary>
        private void Populate(int rActorGuid)
        {
            var propertyCollections = PropertyLoader.GetOrCreate(rActorGuid);
            PropertyLoader.Load<CommonProperties>(this, propertyCollections);

            if (IsExcludedType || IsExcludedId || !IsValid || ActorAttributes == null)
                return;

            switch (ActorType)
            {
                case ActorType.Monster:
                    PropertyLoader.Load<UnitProperties>(this, propertyCollections);
                    break;
                case ActorType.Gizmo:
                    PropertyLoader.Load<GizmoProperties>(this, propertyCollections);
                    break;
                case ActorType.Item:
                    PropertyLoader.Load<ItemProperties>(this, propertyCollections);
                    break;
                case ActorType.Player:
                    PropertyLoader.Load<UnitProperties>(this, propertyCollections);
                    PropertyLoader.Load<PlayerProperties>(this, propertyCollections);
                    break;
            }
        }
