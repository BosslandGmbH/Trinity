...
namespace Trinity
{
    public static class TargetUtil
    {
        ...
        internal static bool IsEliteTargetInRange(float range = 10f)
        {
            if (range < 5f)
                range = 5f;
            return TrinityPlugin.CurrentTarget != null && TrinityPlugin.CurrentTarget.IsElite && TrinityPlugin.CurrentTarget.RadiusDistance <= range;
        }
        ...
        internal static bool IsPositionOnMonster(Vector3 position, bool useWeights = true)
        {
            ...
            return TrinityPlugin.Targets.Any(m => m.Weight > 0 && m.IsUnit && m.Position.Distance(position) <= m.Radius * 0.85);
        }
